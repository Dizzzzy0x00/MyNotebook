...
**（1）非过滤器类防御（Non-filter Defenses）：**&#x76F4;接改变生成过程，不是简单过滤，代表性方法

| 方法                                    | 思想                   |
| ------------------------------------- | -------------------- |
| **UCE (Unlearnable Concept Erasure)** | 从模型内部消除概念，不靠过滤器      |
| **SafeGen**                           | 对生成分布做投影，让模型难生成 NSFW |
| **POSI**                              | 重写 prompt，使其安全       |

**（2）越狱防御（Jailbreak Defenses）**

| 方法              | 思想                                      |
| --------------- | --------------------------------------- |
| **PPL Defense** | 用 perplexity 检测奇怪、不自然 prompt            |
| **SmoothLLM**   | 对 prompt 加噪声（swap/insert/patch），降低越狱稳定性 |
