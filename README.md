...
在传统信号处理中，滤波器用来调整信号在不同频率成分上的幅度。同样，在图卷积中，$$g_{\theta}(\Lambda)$$ 用来调整图信号在图拉普拉斯特征空间（频谱域）中各个频率成分的权重。例如，可以设计滤波器使得低频成分（通常代表图的平滑结构信息）增强，而高频成分（通常与噪声或局部细节相关）被抑制。

**参数化的滤波器**

函数 $$g_{\theta}$$ 通常依赖于一组参数 $$\theta$$，这些参数在训练过程中会被学习到。常见的做法包括：

*   **多项式近似**：例如使用切比雪夫多项式将 $$g_{\theta}(\lambda)$$ 表示为一个多项式

    $$
    g_{\theta}(\lambda) \approx \sum_{k=0}^{K} \theta_k\, T_k(\tilde{\lambda}),
    $$

    其中 $$T_k$$ 是切比雪夫多项式，$$\tilde{\lambda}$$ 是归一化后的特征值。
*   **一阶近似**：
